...
function scrollToPoster(posterName) {
    ...
    if (element) {
        ...
        element.style.boxShadow = '0 0 0 3px #0066CC';
        setTimeout(() => {
...
function generateHTML(groupedComments) {
    ...
    Object.entries(groupedComments).forEach(([posterName, comments]) => {
        ...
        posterSection.id = `poster-${posterName.replace(/\s/g, '-')}`;
        ...
        posterHeader.innerHTML = `
            ...
            <span class="comment-count" data-poster="${posterName}">${comments.length}件</span>
        `;
        ...
        comments.forEach((comment) => {
            ...
            commentItem.setAttribute('data-poster', posterName);
...
async function markAsComplete(rowIndex, button, commentItem) {
    ...
    try {
        ...
        if (remainingItems.length === 0) {
            ...
            // 検索データを更新
            allPosters = allPosters.filter(p => p.name !== posterName);
            
            // 全体が空になったかチェック
            const allSections = document.querySelectorAll('.poster-section');
            ...
            countElement.textContent = `${remainingItems.length}件`;
            
            // 検索データのカウントを更新
            const posterData = allPosters.find(p => p.name === posterName);
            if (posterData) {
                posterData.count = remainingItems.length;
            }
        }
...
function updateStatsAfterComplete() {
    ...
    // 投稿者数も更新
    const posterSections = document.querySelectorAll('.poster-section');
    document.getElementById('totalPosters').textContent = posterSections.length;
}
...
// グローバルに関数を公開
window.scrollToPoster = scrollToPoster;
